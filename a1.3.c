...
void *single_threaded_merge_sort(void *ptr){
    ...
    if (block->size > SPLIT) {
        ...
        single_threaded_merge_sort(&left_block);
        single_threaded_merge_sort(&right_block);
...
void *merge_sort(void *ptr) {
    ...
    if (block->size > SPLIT) {
        ...
        pthread_mutex_lock(&mutex);
        int result = pthread_create(&thread1, NULL, merge_sort, &left_block);
        if(result == 0)
            count++;
        pthread_mutex_unlock(&mutex);
...
int main(int argc, char *argv[]) {
    ...
    free(block.data);
    printf("created this many threads: %d", count);
    exit(EXIT_SUCCESS);
}